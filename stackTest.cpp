...
void postfixTest() {
    ...
    while (1) {
        ...
        if (operandStack.isEmpty()) {
            cout << "The post-fix expression results in " << total << endl;
        } else {
            cout << "The post-fix expression is illegal " << endl;
        }
        else
        {
            if (isdigit(inputHolder)) {
                operandStack.push(inputHolder - '0');
            } else {
                int one, two;
                if (operandStack.isEmpty() == false) {
                    one = operandStack.pop();
                } else {
                    cout << "Error! Operands not acceptable. " << endl;
                    break;
                }
                if (operandStack.isEmpty() == false)
                    two = operandStack.pop();
            }
            else{
                cout << "Error! Operands not acceptable. " << endl;
                break;
